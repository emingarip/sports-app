...
for (const game of games) {
    ...
    // Regex to match the if (data) { setTopScores(data.map(...) } block
    // We'll replace it with our new logic.
    const searchRegex = /if\s*\(data\)\s*\{\s*setTopScores\(data\.map\(\(d:\s*any\)\s*=>\s*\(\{\s*id:\s*d\.id,\s*score:\s*d\.score,\s*username:\s*d\.users\?\.username\s*\|\|\s*'Anonim'\s*\}\)\)\);\s*\}/s;
