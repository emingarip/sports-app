...
interface HeaderHeroProps { readonly roomId: string; readonly gameId: string; }
...
export default function HeaderHero({ roomId, gameId }: HeaderHeroProps) {
  ...
  const screenWidth = globalThis.window !== undefined ? globalThis.window.innerWidth : 300;
  const screenHeight = globalThis.window !== undefined ? globalThis.window.innerHeight : 500;
  ...
  useEffect(() => {
    ...
    const spawnObject = (time: number) => {
      ...
      if (time - lastSpawn.current > currentSpawnRate) {
        ...
        const targetXPos = (screenWidth * 0.5) + (Math.random() * 160 - 80);
        const flightTime = isArc ? 85 : 70;
        ...
        objects.current.push({
          ...
          y: startY,
          dx: calculatedDx,
          dy: (isArc ? -(9 + Math.random()*4) : -(5 + Math.random()*3)) * speedMult,
          gravity: (isArc ? 0.25 : 0.15) * speedMult,
          active: true,
          isGold,
          isHelmet: isHelmetFlag,
          isArc
        });
        
        const newLevel = difficultyScaler.current.getLevel(scoreRef.current);
        if (newLevel !== level) setLevel(newLevel);
      }
    };

    const updateAndDrawObjects = (ctx: CanvasRenderingContext2D, currentY: number, frames: number) => {
      for (let i = objects.current.length - 1; i >= 0; i--) {
        ...
        drawGameObject(ctx, obj, frames);
        checkCollisions(ctx, obj, i, currentY);
      }
    };

    const drawGameObject = (ctx: CanvasRenderingContext2D, obj: GameObject, frames: number) => {
      if (obj.type === 'ball') {
    ...
    const checkCollisions = (ctx: CanvasRenderingContext2D, obj: GameObject, index: number, currentY: number) => {
      const dist = Math.sqrt(Math.pow(obj.x - player.current.x, 2) + Math.pow(obj.y - (currentY - 20), 2));
      if (dist < 35) {
        obj.active = false;
        handleObjectCollision(ctx, obj);
      }
    ...
    const handleObjectCollision = (ctx: CanvasRenderingContext2D, obj: GameObject) => {
       if (obj.isHelmet) {
         ...
         handleHazardCollision(ctx, obj);
       }
    };

    const handleHazardCollision = (ctx: CanvasRenderingContext2D, obj: GameObject) => {
    ...
    const handleMissedObject = (obj: GameObject) => {
    ...
    const updatePlayer = (target: number) => {
      player.current.x += (target - player.current.x) * 0.2;
    ...
    const getPlayerY = () => {
      ...
      if (player.current.isJumping) {
        player.current.jumpProgress += 0.15;
        if (player.current.jumpProgress >= Math.PI) {
          ...
          currentY = player.current.y - Math.sin(player.current.jumpProgress) * 120;
        }
      }
      return currentY;
    };

    const drawPlayer = (ctx: CanvasRenderingContext2D, currentY: number) => {
      ctx.fillStyle = impactFrame.current > 0 ? 'black' : '#fca5a5';
      ctx.beginPath();
      ctx.arc(player.current.x, currentY - 20, 15, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = impactFrame.current > 0 ? 'black' : '#ef4444';
    ...
    const render = (time: number) => {
      ...
      updatePlayer(targetX.current);
      const currentYValue = getPlayerY();

      drawPlayer(ctx, currentYValue);
      spawnObject(time);
      updateAndDrawObjects(ctx, currentYValue, Math.floor(time / 16));
  ...
  const playAgain = () => {
    ...
    objects.current = []; livesRef.current = 3; setLives(3); setLevel(1); lastSpawn.current = 0;
  };
  ...
  const minTime = Math.floor(overallTimeLeft / 60); 
  const secTime = overallTimeLeft % 60;
  ...
  return (
    ...
    >
      <div className="absolute top-4 right-4 z-10 flex flex-col items-end gap-2 pointer-events-none">
          ...
          {topScores.length === 0 ? <p className="text-white/50 text-xs">Henüz skor yok</p> : (
             <div className="space-y-1">
               {topScores.map((ts, idx) => {
                 ...
                 else if (idx === 1) colorClass = 'text-gray-300';
                 else if (idx === 2) colorClass = 'text-orange-300';
                 
                 return (
                   <div key={ts.id} className="flex justify-between items-center text-sm">
                     <span className={`font-medium ${colorClass}`}>{idx + 1}. {ts.username.slice(0, 10)}</span>
        ...
        <div className="bg-black/40 backdrop-blur-sm rounded-xl p-3 border border-white/20 shadow-xl min-w-[140px]">
           ...
           <div className="flex justify-between items-center mb-1">
             ...
             <span className="text-white font-mono font-bold text-lg">{minTime}:{secTime < 10 ? '0' + secTime : secTime}</span>
           </div>
      ...
      {isSubmitting && (
        <div className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-black/40 backdrop-blur-md text-white gap-4">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-white"></div>
          <span className="font-medium tracking-widest animate-pulse uppercase">Sonuçlar Kaydediliyor...</span>
        </div>
      )}
    </div>
  );
