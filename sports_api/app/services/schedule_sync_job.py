...
class ScheduleSyncJobManager:
    def __init__(self, *, session_factory=async_session_factory) -> None:
        self._session_factory = session_factory
        self._status = ScheduleSyncJobStatus()
        self._task: asyncio.Task[None] | None = None
        self._lock = asyncio.Lock()

    async def snapshot(self) -> ScheduleSyncJobStatus:
    ...
    ) -> ScheduleSyncJobStatus:
        async with self._lock:
            if self._task is not None and not self._task.done():
                self._append_event_locked(
                    f"Sync already running for {self._status.provider_slug} {self._status.target_date}."
                )
                return self._copy_status_locked()
            ...
            self._status = ScheduleSyncJobStatus(
                ...
                last_message=f"Sync started for {target_date.isoformat()}.",
            )
            self._append_event_locked(
                f"Sync started provider={provider_slug} date={target_date.isoformat()} tz={timezone_name or '-'}."
            )
        ...
        *,
        ...
        try:
            ...
            async with self._lock:
                ...
                self._status.last_message = f"Sync crashed: {exc}"
                self._append_event_locked(f"Sync crashed: {exc}")
        finally:
            async with self._lock:
                self._task = None
    ...
    def _copy_status_locked(self) -> ScheduleSyncJobStatus:
...
schedule_sync_job_manager = ScheduleSyncJobManager()
